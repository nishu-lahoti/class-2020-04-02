...
Intercept: The intercept is -6.6%, meaning that when less_than_hs is 0%, nobody in the county has less than HS education. The predicted percentage of the
population living in poverty is 6.6%.
Slope: The slope is the estimate of less_than_hs. The slope means that in the model, for each 1% point increase in the population with less
than a hs education, there are an additional 0.64% points of people living in poverty in the country. 
...
# Created a linear regression for each of our bootstrap samples by replicating the code we used to create
# poverty model into a mapping function, assigning it to the variable mod. We then piped the results
# from mod to create a new variable, reg_results, which creates a regression table off the replicates of linear regression
# models we just created. Finally piped these results to create another column, disp_coef, which uses the reg_results
# column to gather the less_than_hs variable within each replication and pulls this value.
